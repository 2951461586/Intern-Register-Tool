...
用法：
    # 先看订阅里有哪些节点（不发任何注册请求）
    python tools/ops/gen_mihomo_slots.py --sub <订阅名> --list-nodes

    # 生成 8 个槽位（每个槽位一个不同节点）
    python tools/ops/gen_mihomo_slots.py --sub <订阅名> --slots 8

    # 只挑美国节点
    python tools/ops/gen_mihomo_slots.py --sub <订阅名> --slots 12 --filter "(?i)美国|United States|US"
