...
用法：
    python tools/gates/install_hooks.py            # 安装 / 覆盖
    python tools/gates/install_hooks.py --uninstall
    python tools/gates/install_hooks.py --check    # 只看装没装
...
MARK = "# ── leak-gate (由 tools/gates/install_hooks.py 生成，勿手工改) ──"
...
if [ ! -x "$PY" ] && ! command -v "$PY" >/dev/null 2>&1; then
  echo "⚠ 找不到 python，跳过泄漏闸门（请手工跑 tools/gates/check_leaks.py）" >&2
  exit 0
fi
"""
...
{probe}
"$PY" tools/gates/check_leaks.py --staged || {
  echo "" >&2
...
{probe}
"$PY" tools/gates/check_leaks.py --history || {
  echo "" >&2
...
def install() -> int:
    ...
    print("\n✓ 钩子已就位。注意它们**不随仓库分发** ——")
    print("  新 clone 后要自己再跑一次 `python tools/gates/install_hooks.py`。")
