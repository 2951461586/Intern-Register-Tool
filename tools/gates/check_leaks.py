...
* **项目自带模式**：自动读 `.env`，把你**真实在用的**值（Worker 子域、出口 IP、
  ...
  不需要另外维护一份清单。
* **闸门必须能失败**：`tools/gates/selftest_check_leaks.py` 做变异验证 ——
  塞假凭据进去必须被拦并点名文件；干净树必须通过。
  "拿旧代码跑一遍通过"只证明它不过度拦截，**完全没有证明它会拦**。

用法
----
    python tools/gates/check_leaks.py             # 已跟踪 + 未跟踪未被忽略的文件（默认）
    python tools/gates/check_leaks.py --staged    # 只查暂存区（pre-commit 钩子用）
    python tools/gates/check_leaks.py --all       # 工作区全部文件（含被忽略的，最严）
    python tools/gates/check_leaks.py --history   # 连**全部 git 历史**一起扫（发布前用）
    python tools/gates/check_leaks.py --quiet     # 只输出结论
...
# 🔴 ROOT 必须指向**仓库根**：错一层就会指向仓库的父目录 → 所有 isdir 判断失败
#    被静默跳过 → 扫描范围变空 → "全 0 命中"的假绿。这是经典 off-by-one。
#
#    2026-09-19 实测踩到过：本文件从 `tools/` 移进 `tools/gates/` 时，
#    原来的 `Path(__file__).resolve().parents[1]` 就从"仓库根"变成了"tools/"。
#    所以现在**不再由本文件自己算**，统一从 `_path` 取 ——
#    "仓库根怎么算"只有 `tools/_bootstrap.py` 一处实现。
#    （下面 `main()` 里仍保留 `.git` 存在性校验，让这类错层大声失败而不是假绿。）
