...
用法：
    python tools/proxypool_ctl.py status      # 体检：谁是谁、端口在哪
    python tools/proxypool_ctl.py stop        # 只停我们的实例
    python tools/proxypool_ctl.py start       # 起槽位实例（用 config.yaml）
    python tools/proxypool_ctl.py start --config .workbuddy-ai/proxypool/test_proxy_ref.yaml
    python tools/proxypool_ctl.py restart
...
def cmd_start(config: Path, workdir: Path, wait: float = 6.0) -> int:
    ...
    if not config.is_file():
        print(f"✗ 找不到配置：{config}\n"
              f"  先生成：python tools/gen_mihomo_slots.py --sub <订阅名> --slots N",
              file=sys.stderr)
        return 1
