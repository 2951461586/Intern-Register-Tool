...
def probe_and_login(label, extra_args, account, password):
    ...
    import src.browser as bl

    # 🔴 自定义启动参数走**显式参数** `chrome_args`，不要去改 `bl.CHROME_ARGS` 全局。
    #    旧写法 `bl.CHROME_ARGS = ...` 依赖"模块属性赋值 == 改该模块的全局变量"。
    #    ⚠ 该写法**已经失效**（2026-09-19 阶段 B 拆包后）：
    #      `_launch_kwargs()` 现在在 `src/browser/session.py`，读的是**它自己命名空间**
    #      里的 `CHROME_ARGS`；改 `bl.CHROME_ARGS` 只会改包级那份副本，
    #      真源不变，而且**不报错** —— 探针会安静地给出失真的结论。
    #    理由见 `src/browser/session.py` 的 `_launch_kwargs()` docstring。
    t0 = time.time()
    res = bl.login(account, password, headless=True, timeout=90,
                   attempts=1, verbose=True, chrome_args=BASE_ARGS + extra_args)
    dt = time.time() - t0
