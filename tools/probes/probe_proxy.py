...
----
    # 单条
    python tools/probes/probe_proxy.py 203.0.113.30:764:USERNAME:PASSWORD
    # 多条（每行一条，支持 host:port:user:pass 或完整 URL）
    python tools/probes/probe_proxy.py --file proxies.txt
    # 只看目标域名可达性、跳过地理查询（更快）
    python tools/probes/probe_proxy.py --no-geo <proxy>
"""
...
from src import redact  # noqa: E402  （脱敏助手；必须在 sys.path 调整之后）
...
def main() -> int:
    ...
    for r in results:
        ...
        print(f"代理   : {redact.redact_url(r['url'])}")
        print(f"判定   : {r['verdict']}")
        print(f"出口 IP: {r['egress_ip'] or '—'}   {loc}")
        if r["note"]:
            print(f"备注   : {r['note']}")
        if r["controls"]:
            print("对照组 : " + "  ".join(f"{k}={v}" for k, v in r["controls"].items()))
        print("目标   : " + "  ".join(f"{k}={v}" for k, v in r["targets"].items()))
    print("=" * 78)
    ...
    if ok:
        ...
        for r in ok:
            print(f"     代理 {redact.redact_url(r['url'])}   →  IR_PROXY=<完整串>")
    ...
    if args.out:
        ...
        if args.no_redact:
            ...
            payload = [
                {**r, "proxy": redact.redact_url(r.get("url", r.get("proxy", ""))),
                 "url": redact.redact_url(r.get("url", ""))}
                for r in results
            ]
