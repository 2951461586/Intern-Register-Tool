...
现在有了槽位（`tools/gen_mihomo_slots.py` + `src/proxypool.py`），
...
用法：
    python tools/probe_register_ip.py --slots .workbuddy-ai/proxypool/slots.txt
    python tools/probe_register_ip.py --proxy http://127.0.0.1:7901 --limit 2
    python tools/probe_register_ip.py --slots-file ... --dry-run   # 只看出口 IP，不发注册
"""
...
def exit_ip(proxy: str, *, timeout: int = 20) -> str:
    ...
    连任何域名都可能"0.02s 连通"（连的是本地虚拟网卡）。见 `tools/probe_proxy.py`。
...
def register_once(proxy: str) -> dict:
    ...
    from src.pipeline import is_quota_block, gen_password
...
def main() -> int:
    ...
    if not slots:
        print(f"✗ 没有槽位。先跑 tools/gen_mihomo_slots.py 生成 {DEFAULT_SLOTS}，"
              f"或用 --proxy 指定。")
        return 1
    if args.mail_domain:
        from src import config
    ...
    if ok:
        ...
        print(f"   → 槽位方案对症。下一步：把 IR_PROXY_SLOTS 配进 .env 跑批量。")
    elif blocked and not other:
        print(f"\n❌ **换了 {len(blocked)} 个不同出口 IP 仍全部 B0000。**")
        print(f"   说明封禁**不只是** IP 维度（可能还有账号/域名/全局维度）。")
        print(f"   换 IP 不是对症解法 —— 别再往这个方向投入了。")
    elif blocked:
