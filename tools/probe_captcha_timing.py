...
----
    # 实验组（微移动开，即生产默认）
    python .workbuddy-ai/tmp/probe_captcha_timing.py --mode on --rounds 3

    # 对照组（微移动关，纯等待）
    IR_NO_MICRO_MOVE=1 python .workbuddy-ai/tmp/probe_captcha_timing.py --mode off --rounds 3
"""
...
from src import browser_login  # noqa: E402
...
def main():
    ...
    flag_ok = (browser_login.MICRO_MOVE == (args.mode == "on"))
    if args.budget is not None and args.mode == "on":
        flag_ok = flag_ok and abs(browser_login.MICRO_BUDGET_S - args.budget) < 1e-6
    print(f"[probe] mode={args.mode}  MICRO_MOVE={browser_login.MICRO_MOVE}  "
          f"budget={browser_login.MICRO_BUDGET_S}s  一致={flag_ok}", flush=True)
    if not flag_ok:
    ...
    for i in range(args.rounds):
        ...
        t0 = time.time()
        res = browser_login.login(rec.email, rec.password, headless=False,
