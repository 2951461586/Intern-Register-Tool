...
----
    # 对照组：当前生产配置
    python tools/probe_login_timing.py --type-lo 45 --type-hi 110 \\
        --rounds 5 --drop-first

    # 实验组：打字调快
    python tools/probe_login_timing.py --type-lo 15 --type-hi 40 \\
        --rounds 5 --drop-first
...
def main():
    ...
    checks = [("PREWARM_MS", browser_login.PREWARM_MS, args.prewarm)]
    if args.type_lo is not None:
        checks.append(("TYPE_DELAY_LO", browser_login.TYPE_DELAY_LO, args.type_lo))
    if args.type_hi is not None:
        checks.append(("TYPE_DELAY_HI", browser_login.TYPE_DELAY_HI, args.type_hi))
    ...
    for i in range(args.rounds):
        ...
        res = browser_login.login(rec.email, rec.password, headless=True,
