...
用法
----
    python tools/export_keys.py                        # 用默认来源
    python tools/export_keys.py --source a.json --source b.zip
    python tools/export_keys.py --out /tmp/keys
"""
...
def main() -> int:
    ...
    if not args.force and not _is_git_ignored(out):
        ...
        print(f"  输出含**明文 key**，且 *.csv / *.txt 并不在 .gitignore 里 ——")
        print(f"  写到那里会变成可提交的凭据。改用默认目录，或加 --force 自负风险。")
        return 2
