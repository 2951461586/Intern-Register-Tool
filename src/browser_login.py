...
# ─────────────────────────────────────────────────────────────────
# 逐字输入的按键间隔（毫秒）—— 已实测：**调小不省时间，别动**
# ─────────────────────────────────────────────────────────────────
# 文件头原写"打字延迟是行为信号，不要调小"。那是推断。实测（各 3 轮，6/6 成功）：
#
#   组            typed     captcha_wait   登录总耗时    通路
#   45~110ms     4.87s       5.37s          16.56s     3/3 Path A
#   20~55ms      2.94s       7.26s          16.58s     3/3 Path A
#                (−1.93s)    (+1.89s)       (≈ 0)
#
# 🔴 **打字省下的时间被验证码等待原样吃回去，净收益 ≈ 0（-0.02s）。**
#
# 机制：TRACELESS 有一个**按页面加载时刻起算的最小收集窗口**，不按我们
# 何时提交起算。提前打完字只是把等待挪到后面 —— 总时长由窗口决定，不由打字速度决定。
#
# → 所以**默认值保持 45~110ms**（更接近真人，且没有任何性能代价）。
